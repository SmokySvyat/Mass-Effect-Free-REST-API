...
| GET | ```contacts/all``` | Возвращает массив объектов "безопасных" контактов (без значения пароля). Пароль хэшируется и хранится в коллекции в зашифрованном виде, без возможности его просмотреть. |
|--:|:--:|:--|

| GET | ```contacts/:id``` | Возвращает объект "безопасного" контакта по запрашиваемому Id. Запрос контакта по несуществующему Id возвращает объект с сообщением. |
|--:|:--:|:--|
